...
export default function SettingsPage() {
  ...
  const [loadingPaymentPin, setLoadingPaymentPin] = useState(false);
  ...
  async function handleSetPaymentPin() {
    try {
      ...
      if (!response.ok) {
        toast.error(data.message || "Failed to set payment PIN");
        return;
        }
  ...
  async function handleResetPaymentPin() {
    try {
      ...
      const response = await fetch("/api/settings/reset-pin", {
        method: "POST",
      });
      const data = await response.json();

      if (!response.ok) {
         toast.error(data.message || "Failed to send verification token");
        return;
      }
  ...
  async function handleVerifyResetPin() {
    try {
      if (!verificationToken || !newResetPin || !confirmResetPin) {
      ...
      const response = await fetch("/api/settings/verify-reset-pin", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      toast.success("Payment PIN reset successful");
      setVerificationToken("");
      setNewResetPin("");
      setConfirmResetPin("");
      setOpenVerifyPinModal(false);
    ...
    return (
      <div className="min-h-screen bg-zinc-50 px-4 py-6 dark:bg-zinc-950 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-6xl space-y-6">
          <div className="h-44 animate-pulse rounded-[2rem] bg-zinc-200 dark:bg-white/10" />
          <div className="grid gap-5 lg:grid-cols-2">
            {Array.from({ length: 6 }).map((_, index) => (
              <div
                key={index}
                className="h-56 animate-pulse rounded-[1.75rem] bg-zinc-200 dark:bg-white/10"
              />
            ))}
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-white px-3 py-5 text-zinc-950 dark:bg-zinc-950 dark:text-white sm:px-1 lg:pl-10 lg:px-1">
      <div className="mx-auto  space-y-6">
         <section className="relative overflow-hidden rounded-[2rem] border border-zinc-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-white/[0.04] sm:p-8">
          ...
          <div className="absolute bottom-0 left-0 h-52 w-52 rounded-full bg-amber-500/10 blur-3xl" />

          <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700 dark:border-emerald-500/20 dark:bg-emerald-500/10 dark:text-emerald-300">
              ...
              <p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-500 dark:text-zinc-400">
                Manage your profile, security preferences, notifications, and
                transaction PIN from one secure place.
              </p>
            </div>
         ...
         <section className="grid gap-5 lg:grid-cols-2">
          <Card className="rounded-[1.75rem] border-zinc-200 bg-white shadow-sm dark:border-white/10 dark:bg-white/[0.04]">
            <CardHeader>
              <div className="flex items-center gap-3">
                <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-zinc-100 text-zinc-700 dark:bg-white/10 dark:text-zinc-200">
                  <User className="h-5 w-5" />
                </div>
                <div>
                  <CardTitle>Profile Information</CardTitle>
            ...
            <CardContent className="space-y-5">
              ...
              <Button
                onClick={handleSaveProfile}
                disabled={loadingProfile}
                className="h-12 rounded-2xl bg-zinc-950 px-5 text-white hover:bg-zinc-800 dark:bg-white dark:text-zinc-950 dark:hover:bg-zinc-200"
              >
                {loadingProfile ? (
                  <>
                    <Loader2 className="mr-2 h-4 w-4 animate-spin" />
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input
                    ...
                    value={currentPassword}
                    onChange={(e) => setCurrentPassword(e.target.value)}
                    className="h-12 rounded-2xl border-zinc-200 bg-zinc-50 pr-12 dark:border-white/10 dark:bg-zinc-950"
                  />
                  <button
                    type="button"
                    onClick={() => setShowPassword(!showPassword)}
                    className="absolute right-4 top-1/2 -translate-y-1/2 text-zinc-500 transition hover:text-zinc-950 dark:hover:text-white"
                  >
                    {showPassword ? (
                      <EyeOff className="h-5 w-5" />
                    ) : (
          ...
          <Card className="rounded-[1.75rem] border-zinc-200 bg-white shadow-sm dark:border-white/10 dark:bg-white/[0.04]">
            ...
            <CardContent>
               {mounted && (
                <div className="flex items-center justify-between rounded-2xl border border-zinc-200 bg-zinc-50 p-4 dark:border-white/10 dark:bg-zinc-950/70">
                  <div>
                    <p className="font-medium">Dark Mode</p>
           ...
           <Card className="rounded-[1.75rem] border-zinc-200 bg-white shadow-sm dark:border-white/10 dark:bg-white/[0.04]">
            <CardHeader>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <CardTitle>Notifications</CardTitle>
                  <CardDescription>Manage transaction alerts.</CardDescription>
                </div>
              </div>
            </CardHeader>
            ...
            <CardHeader>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <CardTitle>Payment PIN</CardTitle>
                  <CardDescription>
                    Set a secure 4-digit PIN for wallet transactions.
                  </CardDescription>
                </div>
              </div>
            </CardHeader>
              ...
              <div className="grid gap-5 md:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <Input
                      ...
                      className="h-12 rounded-2xl border-zinc-200 bg-zinc-50 pr-12 text-center text-lg tracking-[8px] dark:border-white/10 dark:bg-zinc-950"
                    />
                    <button
                      type="button"
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    value={confirmPaymentPin}
                    onChange={(e) =>
                      setConfirmPaymentPin(e.target.value.replace(/\D/g, ""))
                    }
                    className="h-12 rounded-2xl border-zinc-200 bg-zinc-50 text-center text-lg tracking-[8px] dark:border-white/10 dark:bg-zinc-950"
                  />
                </div>
              </div>
              <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-500/20 dark:bg-amber-500/10">
                <p className="text-sm text-amber-800 dark:text-amber-200">
                  Your payment PIN protects airtime, data, utilities, cable TV,
          ...
          <Card className="rounded-[1.75rem] border-zinc-200 bg-white shadow-sm dark:border-white/10 dark:bg-white/[0.04]">
            <CardHeader>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <CardTitle>Reset Payment PIN</CardTitle>
                  <CardDescription>Send a reset token to your email.</CardDescription>
                </div>
              </div>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="rounded-2xl border border-zinc-200 bg-zinc-50 p-4 text-sm text-zinc-500 dark:border-white/10 dark:bg-zinc-950/70 dark:text-zinc-400">
                Use this only if you cannot remember your current transaction
                PIN.
              </div>
              ...
              <AlertDialog>
                ...
                <AlertDialogContent className="rounded-3xl">
                  <AlertDialogHeader>
                    <AlertDialogTitle>Reset Payment PIN?</AlertDialogTitle>
                    <AlertDialogDescription>
                      We will send a verification token to your email so you can
        ...
        <Card className="rounded-[1.75rem] border-red-200 bg-red-50/70 shadow-sm dark:border-red-500/20 dark:bg-red-500/10">
          <CardHeader>
            <div className="flex items-center gap-3">
      ...
      <Dialog open={openVerifyPinModal} onOpenChange={setOpenVerifyPinModal}>
        <DialogContent className="rounded-3xl border-zinc-200 dark:border-white/10">
          <DialogHeader>
            <DialogTitle>Verify Reset Token</DialogTitle>
            <DialogDescription>
              Enter the verification token sent to your email and create a new
              payment PIN.
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  value={newResetPin}
                  onChange={(e) =>
                    setNewResetPin(e.target.value.replace(/\D/g, ""))
                  }
                  className="h-12 rounded-2xl text-center tracking-[8px]"
                />
              </div>
