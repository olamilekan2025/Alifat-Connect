...
export function AdminChatDashboard() {
  ...
  const [status, setStatus] = useState("all");
  const [updating, setUpdating] = useState<string | null>(null);
  // default true so the list is what admin sees first on mobile
  ...
  return (
    <div className="flex h-[calc(100vh-5rem)] overflow-hidden border bg-background">
      <aside
        className={cn(
          "w-full shrink-0  border-r pt-15  md:flex md:w-96 md:flex-col",
          showSidebar
            ? "fixed inset-0 z-30 flex flex-col md:static md:z-auto"
            : "hidden",
        )}
      >
        <div className="space-y-3 border-b p-4">
      ...
      >
        {/* MOBILE-ONLY top bar so admin always has a way back to the list */}
        <div className="flex h-12 items-center  border-b px-2 md:hidden">
          ...
          <span className="ml-2 truncate text-sm font-medium">
            {currentConversation?.user.name || "Conversations"}
          </span>
        </div>
